...
// std::mt19937_64 and sampling with our own integer helpers (so we do not depend
// on the platform's distribution implementations).
...
class SyntheticGenerator {
 ...
 private:
  // Uniform integer in [lo, hi] from the raw 64-bit stream (deterministic across
  ...
  Price clamp_price(std::int64_t p) const {
    if (p < 1) return 1;
    if (p > cfg_.num_ticks - 2) return cfg_.num_ticks - 2;
